...
#pragma once
#include "SWseg_Self_Analysis.hpp"

/*
SWmeta used for detailed self-analysis
*/
...
private:
    ...
    void exponential_search_right(Type_Key & targetKey, int & foundPos);
    void binary_search_right(Type_Key & targetKey, int & foundPos, int maxSearchBound);
    void exponential_search_right_insert(Type_Key & targetKey, int & foundPos, int maxSearchBound);
    void exponential_search_right_insert(Type_Key & targetKey, int & foundPos);
    void binary_search_right_insert(Type_Key & targetKey, int & foundPos, int maxSearchBound);
    void exponential_search_left(Type_Key & targetKey, int & foundPos, int maxSearchBound);
    void exponential_search_left(Type_Key & targetKey, int & foundPos);
    void binary_search_left(Type_Key & targetKey, int & foundPos, int maxSearchBound);
    void exponential_search_left_insert(Type_Key & targetKey, int & foundPos, int maxSearchBound);
    void exponential_search_left_insert(Type_Key & targetKey, int & foundPos);
    void binary_search_left_insert(Type_Key & targetKey, int & foundPos, int maxSearchBound);
...
public:
    ...
    void print();
    void print_all();
    int get_max_root_error();
    int get_right_root_error();
    int get_left_root_error();
    double get_avg_seg_error();
    double get_avg_right_seg_error();
    double get_avg_left_seg_error();
...
void SWmeta<Type_Key,Type_Ts>::bulk_load(const vector<pair<Type_Key, Type_Ts>> & stream)
{
    ...
    if (m_slope != -1)
    {
        ...
        #ifdef LOCAL_SEARCH
            #if LOCAL_SEARCH == 1
            m_maxSearchError = numeric_limits<int>::max();
            #endif
        #else
        #endif
    }
...
{  
    ...
    vector<tuple<int,int,double>> splitIndexSlopeVector;
        ...
        #elif (BULKLOAD == 1)
        calculate_split_index_shrinking_cone(data, splitIndexSlopeVector);
        #elif (BULKLOAD == 2)
        calculate_split_equal(128, data, splitIndexSlopeVector);
        #endif
    #else
    calculate_split_derivative(128, data, splitIndexSlopeVector);
    #endif
        ...
        {   
            ...
            if (predictPosMin < predictPosMax)
            {
                ...
                if (insertKeyPtr.first < m_keys[insertionPos]) // Left of predictedPos
                {
                        ...
                        #elif (LOCAL_SEARCH == 1)
                        exponential_search_left_insert(insertKeyPtr.first, insertionPos);
                        #elif (LOCAL_SEARCH == 2)
                        binary_search_left_insert(insertKeyPtr.first, insertionPos, predictPos-predictPosMin);
                        #endif
                    #else
                    exponential_search_left_insert(insertKeyPtr.first, insertionPos, predictPos-predictPosMin);
                    #endif
                ...
                else if (insertKeyPtr.first > m_keys[insertionPos]) //Right of predictedPos
                {
                        ...
                        #elif (LOCAL_SEARCH == 1)
                        exponential_search_right_insert(insertKeyPtr.first, insertionPos);
                        #elif (LOCAL_SEARCH == 2)
                        binary_search_right_insert(insertKeyPtr.first, insertionPos, predictPosMax-predictPos);
                        #endif
                    #else
                    exponential_search_right_insert(insertKeyPtr.first, insertionPos, predictPosMax-predictPos);
                    #endif
    ...
    else
    {
        ...
        if (predictPosMin < predictPosMax)
        {
            ...
            if (targetKey <  m_keys[predictPos]) // Left of predictedPos
            {
                    ...
                    #elif (LOCAL_SEARCH == 1)
                    exponential_search_left(targetKey, foundPos);
                    #elif (LOCAL_SEARCH == 2)
                    binary_search_left(targetKey, foundPos, predictPos-predictPosMin);
                    #endif
                #else
                exponential_search_left(targetKey, foundPos, predictPos-predictPosMin);
                #endif
            ...
            else if (targetKey > m_keys[predictPos]) //Right of predictedPos
            {
                    ...
                    #elif (LOCAL_SEARCH == 1)
                    exponential_search_right(targetKey, foundPos);
                    #elif (LOCAL_SEARCH == 2)
                    binary_search_right(targetKey, foundPos, predictPosMax-predictPos);
                    #endif
                #else
                exponential_search_right(targetKey, foundPos, predictPosMax-predictPos);
                #endif
...
template<class Type_Key, class Type_Ts>
inline void SWmeta<Type_Key,Type_Ts>::exponential_search_right(Type_Key & targetKey, int & foundPos)
{
    #ifdef TUNE
    int predictPos = foundPos;
    #endif
    ...
    int index = 1;
    while (foundPos + index < m_keys.size() && m_keys[foundPos + index] < targetKey)
    {
        index *= 2;
    }

    auto startIt = m_keys.begin() + (foundPos + static_cast<int>(index/2));
    auto endIt = m_keys.begin() + min(foundPos + index, (int)m_keys.size());
...
template<class Type_Key, class Type_Ts>
inline void SWmeta<Type_Key,Type_Ts>::binary_search_right(Type_Key & targetKey, int & foundPos, int maxSearchBound)
{
    #ifdef TUNE_TIME
    int predictPos = foundPos;
    uint64_t temp = 0;
    ...
    auto startIt = m_keys.begin() + foundPos;
    auto endIt = m_keys.begin() + min(foundPos + maxSearchBound, (int)m_keys.size());
    ...
    #ifdef TUNE
    rootNoExponentialSearch++;
    rootLengthExponentialSearch += maxSearchBound;
    #endif
    ...
    {
        rootExponentialSearchCyclePerLength += (double)temp/(maxSearchBound);
    }
    #endif
}
...
template<class Type_Key, class Type_Ts>
inline void SWmeta<Type_Key,Type_Ts>::exponential_search_right_insert(Type_Key & targetKey, int & foundPos)
{
    #ifdef TUNE
    int predictPos = foundPos;
    #endif
...
template<class Type_Key, class Type_Ts>
inline void SWmeta<Type_Key,Type_Ts>::binary_search_right_insert(Type_Key & targetKey, int & foundPos, int maxSearchBound)
{
    #ifdef TUNE_TIME
    int predictPos = foundPos;
    uint64_t temp = 0;
...
template<class Type_Key, class Type_Ts>
inline void SWmeta<Type_Key,Type_Ts>::exponential_search_left(Type_Key & targetKey, int & foundPos)
{
    #ifdef TUNE
    int predictPos = foundPos;
    #endif
    ...
    int index = 1;
    while (foundPos - index >= 0 && m_keys[foundPos - index] >= targetKey)
    {
        index *= 2;
    }

    auto startIt = m_keys.begin() + (foundPos - min(index,foundPos));
    auto endIt = m_keys.begin() + (foundPos - static_cast<int>(index/2));
...
template<class Type_Key, class Type_Ts>
inline void SWmeta<Type_Key,Type_Ts>::binary_search_left(Type_Key & targetKey, int & foundPos, int maxSearchBound)
{
    #ifdef TUNE_TIME
    int predictPos = foundPos;
    uint64_t temp = 0;
    ...
    auto startIt = m_keys.begin() + max(foundPos - maxSearchBound,0);
    auto endIt = m_keys.begin() + foundPos;
...
template<class Type_Key, class Type_Ts>
inline void SWmeta<Type_Key,Type_Ts>::exponential_search_left_insert(Type_Key & targetKey, int & foundPos)
{
    #ifdef TUNE
    int predictPos = foundPos;
    #endif
...
template<class Type_Key, class Type_Ts>
inline void SWmeta<Type_Key,Type_Ts>::binary_search_left_insert(Type_Key & targetKey, int & foundPos, int maxSearchBound)
{
...
template<class Type_Key, class Type_Ts>
int SWmeta<Type_Key,Type_Ts>::get_max_root_error()
{
    return max(m_rightSearchBound,m_leftSearchBound);
}

template<class Type_Key, class Type_Ts>
int SWmeta<Type_Key,Type_Ts>::get_right_root_error()
{
    return m_rightSearchBound;
}

template<class Type_Key, class Type_Ts>
int SWmeta<Type_Key,Type_Ts>::get_left_root_error()
{
    return m_leftSearchBound;
}

template<class Type_Key, class Type_Ts>
double SWmeta<Type_Key,Type_Ts>::get_avg_seg_error()
{
    double totalError = 0;
    for (int i = 0; i < m_keys.size(); i++)
    {
        if (bitmap_exists(i))
        {
            totalError += max(m_ptr[i]->m_rightSearchBound,m_ptr[i]->m_leftSearchBound);
        }
    }
    return totalError/(double)m_numPairExist;
}

template<class Type_Key, class Type_Ts>
double SWmeta<Type_Key,Type_Ts>::get_avg_right_seg_error()
{
    double totalError = 0;
    for (int i = 0; i < m_keys.size(); i++)
    {
        if (bitmap_exists(i))
        {
            totalError += m_ptr[i]->m_rightSearchBound;
        }
    }
    return totalError/(double)m_numPairExist;
}
...
template<class Type_Key, class Type_Ts>
double SWmeta<Type_Key,Type_Ts>::get_avg_left_seg_error()
{
    double totalError = 0;
    for (int i = 0; i < m_keys.size(); i++)
    {
        if (bitmap_exists(i))
        {
            totalError += m_ptr[i]->m_leftSearchBound;
        }
    }
    return totalError/(double)m_numPairExist;
}
