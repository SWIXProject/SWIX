...
/*
SWIX used for detailed self-analysis
*/
...
private:
    ...
    void exponential_search_model_right(Type_Key & targetKey, int & foundPos);
    void binary_search_model_right(Type_Key & targetKey, int & foundPos, int maxSearchBound);
    void exponential_search_model_left(Type_Key & targetKey, int & foundPos, int maxSearchBound);
    void exponential_search_model_left(Type_Key & targetKey, int & foundPos);
    void binary_search_model_left(Type_Key & targetKey, int & foundPos, int maxSearchBound);
    ...
    if (m_numPair)
    {
        ...
        if (predictPosMin < predictPosMax)
        {
            ...
            {  
                    ...
                    #elif (LOCAL_SEARCH == 1)
                    exponential_search_model_left(newKey, actualPos);
                    #elif (LOCAL_SEARCH == 2)
                    binary_search_model_left(newKey, actualPos, predictPos-predictPosMin);
                    #endif
                #else
                exponential_search_model_left(newKey, actualPos, predictPos-predictPosMin);
                #endif
            ...
            else if (newKey > m_localData[predictPos].first)
            {
                    ...
                    #elif (LOCAL_SEARCH == 1)
                    exponential_search_model_right(newKey, actualPos);
                    #elif (LOCAL_SEARCH == 2)
                    binary_search_model_right(newKey, actualPos, predictPosMax-predictPos);
                    #endif
                #else
                exponential_search_model_right(newKey, actualPos, predictPosMax-predictPos);
                #endif
    ...
    {     
        ...
        if (lowerBound < m_localData[predictPos].first) // Left of predictedPos
        {
            actualPos = predictPos;
                ...
                #elif (LOCAL_SEARCH == 1)
                exponential_search_model_left(lowerBound, actualPos);
                #elif (LOCAL_SEARCH == 2)
                binary_search_model_left(lowerBound, actualPos, predictPos-predictPosMin);
                #endif
            #else
            exponential_search_model_left(lowerBound, actualPos, predictPos-predictPosMin);
            #endif
        ...
        else if (lowerBound > m_localData[predictPos].first) //Right of predictedPos
        {
            actualPos = predictPos;
                ...
                #elif (LOCAL_SEARCH == 1)
                exponential_search_model_right(lowerBound, actualPos);
                #elif (LOCAL_SEARCH == 2)
                binary_search_model_right(lowerBound, actualPos, predictPosMax-predictPos);
                #endif
            #else
            exponential_search_model_right(lowerBound, actualPos, predictPosMax-predictPos);
            #endif
        ...
        if (predictPosMin < predictPosMax)
        {
            ...
            if (newKey < m_localData[predictPos].first) // Left of predictedPos
            {
                insertionPos = predictPos;
                    ...
                    #elif (LOCAL_SEARCH == 1)
                    exponential_search_model_left(newKey, insertionPos);
                    #elif (LOCAL_SEARCH == 2)
                    binary_search_model_left(newKey, insertionPos, predictPos-predictPosMin);
                    #endif
                #else
                exponential_search_model_left(newKey, insertionPos, predictPos-predictPosMin);
                #endif
            ...
            else if (newKey > m_localData[predictPos].first) //Right of predictedPos
            {
                insertionPos = predictPos;
                    ...
                    #elif (LOCAL_SEARCH == 1)
                    exponential_search_model_right(newKey, insertionPos);
                    #elif (LOCAL_SEARCH == 2)
                    binary_search_model_right(newKey, insertionPos, predictPosMax-predictPos);
                    #endif
                #else
                exponential_search_model_right(newKey, insertionPos, predictPosMax-predictPos);
                #endif
...
template<class Type_Key, class Type_Ts>
inline void SWseg<Type_Key,Type_Ts>::exponential_search_model_right(Type_Key & targetKey, int & foundPos)
{
    #ifdef TUNE
    int predictPos = foundPos;
    #endif
    ...
    int index = 1;
    while (foundPos + index  < m_localData.size() && m_localData[foundPos + index].first < targetKey)
    {
        index *= 2;
    }
    
    auto startIt = m_localData.begin() + (foundPos + static_cast<int>(index/2));
    auto endIt = m_localData.begin() + min(foundPos + index, (int)m_localData.size());
...
template<class Type_Key, class Type_Ts>
inline void SWseg<Type_Key,Type_Ts>::binary_search_model_right(Type_Key & targetKey, int & foundPos, int maxSearchBound)
{
    #ifdef TUNE_TIME
    int predictPos = foundPos;
    uint64_t temp = 0;
    ...
    auto startIt = m_localData.begin() + foundPos;
    auto endIt = m_localData.begin() + min(foundPos+maxSearchBound, (int)m_localData.size());
    ...
    {
        segNoExponentialSearch++;
        segLengthExponentialSearch += maxSearchBound;
    }
    segNoExponentialSearchAll++;
    segLengthExponentialSearchAll += maxSearchBound;
    #endif
    ...
    {
        segExponentialSearchCyclePerLength += (double)temp/(maxSearchBound);
    }
    #endif
}
...
template<class Type_Key, class Type_Ts>
inline void SWseg<Type_Key,Type_Ts>::exponential_search_model_left(Type_Key & targetKey, int & foundPos)
{
    #ifdef TUNE
    int predictPos = foundPos;
    #endif
    ...
    int index = 1;
    while (foundPos - index >= 0 && m_localData[foundPos - index].first >= targetKey)
    {
        index *= 2;
    }
    
    auto startIt = m_localData.begin() + (foundPos - min(index,foundPos));
    auto endIt = m_localData.begin() + (foundPos - static_cast<int>(index/2));
...
template<class Type_Key, class Type_Ts>
inline void SWseg<Type_Key,Type_Ts>::binary_search_model_left(Type_Key & targetKey, int & foundPos, int maxSearchBound)
{
    #ifdef TUNE_TIME
    int predictPos = foundPos;
    uint64_t temp = 0;
    ...
    auto startIt = m_localData.begin() + max(foundPos - maxSearchBound,0);
    auto endIt = m_localData.begin() + foundPos;
