...
template<class key_t, class val_t>
size_t AIDEL<key_t, val_t>::memory_usage_of_entry()
{
    return sizeof(key_t) * model_keys.size() + sizeof(std::vector<key_t>) +
           sizeof(aidelmodel_type) * aimodels.size() + sizeof(std::vector<aidelmodel_type>)+
           sizeof(canonical_segment) * segments.size() + sizeof(std::vector<canonical_segment>)+
...
template<class key_t, class val_t>
size_t AIDEL<key_t, val_t>::memory_usage()
{
    size_t mem = memory_usage_of_entry();


    for(int i=0; i<aimodels.size(); i++){
        mem += aimodels[i].memory_usage_of_aidelmodel();
    }

    
    // for(int i=0; i<aimodels.size(); i++){
    //     mem += aimodels[i].memroy_usage();
    // }
