...
template<class key_t, class val_t>
size_t AidelModel<key_t, val_t>::memory_usage_of_aidelmodel()
{
    ...
    size_t mem = 0;
    mem += sizeof(AidelModel<key_t, val_t>);
    mem += sizeof(key_t)*capacity;
    mem += sizeof(val_t)*capacity;
    mem += sizeof(bool)*capacity;
    mem += sizeof(model_or_bin_t*)*capacity;
    mem += sizeof(size_t)*3;

    mem += sizeof(levelbin_type*)*(capacity+1);
    mem += sizeof(model_or_bin_t*)*(capacity+1);
