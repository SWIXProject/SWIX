...
public:
    ...
    /**
     * NEW FUNCTION not in orginal PGM INDEX to get stats
     * @return a vector with stats of each pgm sub index.
     */
    std::vector<std::tuple<size_t,size_t,size_t>> find_stats() const {
        std::vector<std::tuple<size_t,size_t,size_t>> output;
        for (auto &p: pgms)
            output.push_back(std::make_tuple(p.segments_count(), p.height(), p.size_in_bytes()));
        return output;
    }
