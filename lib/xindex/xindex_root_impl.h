...
template <class key_t, class val_t, bool seq>
size_t Root<key_t, val_t, seq>::memory_usage_of_root() {
  
  size_t mem =0;

  mem+= sizeof(size_t) *2; // group_n, rmi_2nd_stage_model_n
  mem+= sizeof(LinearModel<key_t>);

  mem+= rmi_1st_stage.memory_usage_of_linear_model();

  for (size_t i = 0; i < rmi_2nd_stage_model_n; i++) {
    mem += rmi_2nd_stage[i].memory_usage_of_linear_model();
  }
  // std::cout << "rmi_2nd_stage_model_n = " << rmi_2nd_stage_model_n << std::endl;s

  mem+= sizeof(std::unique_ptr<std::pair<key_t, group_t *volatile>[]>);



  for(int g=0; g<group_n; g++)
  {
    mem+=groups[g].second->memory_usage_of_group();
  }
  // std::cout << "group_n = " << group_n << std::endl;
  // mem

  // std::cout << "rmi_2nd_stage_model_n " << rmi_2nd_stage_model_n << std::endl;
  // mem+= rmi_2nd_stage->memory_usage_of_linear_model();
