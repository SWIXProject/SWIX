...
template <class key_t, class val_t>
size_t AltBtreeBuffer<key_t, val_t>::memory_usage_of_buffer() {
    size_t mem = sizeof(*this);  // Memory used by AltBtreeBuffer object itself

    mem += sizeof(uint8_t *) * allocated_blocks.capacity(); //pointers to allocated blocks

    mem += allocated_blocks.size() * node_n_per_block * node_size; //node data
    // std::cout << "node_size = " << node_size << std::endl;
    mem += size_est * node_size; //buffer nodes
    return mem;
}
