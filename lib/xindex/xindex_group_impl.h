...
template <class key_t, class val_t, bool seq, size_t max_model_n>
size_t Group<key_t, val_t, seq, max_model_n>::memory_usage_of_group()
{
  size_t mem = 0;
  mem += sizeof(key_t);
  mem += sizeof(uint32_t);
  ...
  mem += sizeof(bool);
  mem += sizeof(record_t *);
  mem += sizeof(record_t) * this->capacity;
  mem += sizeof(buffer_t *) * 2;
  // mem += sizeof(buffer_t) * buffer->size();

  // for (int b=0; b < buffer->size(); b++) {
  //   mem += (buffer++)->memory_usage_of_buffer();
  // }
  mem += buffer->memory_usage_of_buffer();

  if(buffer_temp != nullptr)
  {
    mem += buffer_temp->memory_usage_of_buffer();
  }
    

  // std::cout << "buffer->size() = " << buffer->size() << std::endl;
  // std::cout << "buffer_temp->memory_usage_of_buffer() = " << buffer_temp->memory_usage_of_buffer() << std::endl;

  // mem += sizeof(buffer_t) * buffer_temp->size();
  mem += sizeof(double);
  mem += sizeof(int32_t);
  mem += sizeof(volatile uint8_t);


  mem += sizeof(model_info_t) * max_model_n;
