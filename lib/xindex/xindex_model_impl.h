...
template <class key_t>
size_t LinearModel<key_t>::memory_usage_of_linear_model()
{
  size_t mem = 0;
  ...
  mem += sizeof(std::array<double, key_t::model_key_size()>); 
  mem += sizeof(double) * key_t::model_key_size(); //weights


  // std::cout << "key_t::model_key_size(): " << key_t::model_key_size() << std::endl;
